...
// Parse a graphql query string and return an object representing
// all the arguments
const parseQuery = (query) => {
  try {
    const q = schema.graphql.parse(query);
    return map(q.definitions, (d) => {
      return map(d.selectionSet.selections, (s) => {
        const selection = s.name.value;
        const queryObject = {};
        map(s.arguments, (a) => {
          queryObject[a.name.value] = a.value.value;
        });
        return { selection: selection, arguments: queryObject };
      });
    });
  }
  catch(e) {
